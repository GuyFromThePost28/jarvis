...
socket.onMessage((msg) => {
  ...
  if (type === "audio") {
    ...
    if (msg.text) console.log("[Vader]", msg.text);
  } else if (type === "status") {
...
btnFixSelf.addEventListener("click", (e) => {
  e.stopPropagation();
  menuDropdown.style.display = "none";
  // Activate work mode on the WebSocket session (Vader becomes Claude Code's voice)
