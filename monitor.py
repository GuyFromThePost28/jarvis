"""
Vader Conversation Monitor

Watches the Vader server logs in real-time, analyzes conversation quality,
and reports issues that need fixing. Run alongside the Vader server.
...
class ConversationMonitor:
    ...
    def analyze_latest(self):
        ...
        # ── Check Vader responses ──
        if latest["role"] == "vader":
            text = latest["text"]

            # Too long for voice?
            sentences = text.split(". ")
            if len(sentences) > 4:
                self.flag(f"Vader response too long for voice ({len(sentences)} sentences): {text[:80]}...")

            # Generic AI patterns that Vader shouldn't use
            bad_patterns = [
                ("How can I help", "Vader doesn't ask 'how can I help' — he just acts"),
                ("Is there anything else", "Vader doesn't ask 'is there anything else'"),
                ("I'd be happy to", "Too corporate — Vader says 'Will do, sir' or just does it"),
                ("Absolutely!", "Vader doesn't use filler enthusiasm"),
                ("Great question", "Vader never says 'great question'"),
                ("I don't have access", "Vader should say 'I'm afraid I don't have that information, sir'"),
                ("As an AI", "Vader never breaks character"),
                ("I cannot", "Vader says 'I'm afraid that's beyond my current capabilities, sir'"),
            ]
            ...
            vader_msgs = [m for m in self.messages if m["role"] == "vader"]
            if len(vader_msgs) >= 5:
                recent = vader_msgs[-5:]
                sir_count = sum(1 for m in recent if "sir" in m["text"].lower())
                if sir_count < 1:
                    self.flag("Vader hasn't said 'sir' in the last 5 responses — should use it more")
            ...
            if prev and prev["role"] == "user":
                ...
                if any(w in user_text for w in ["earlier", "before", "you said", "we talked about", "remember"]):
                    if "I don't recall" in text or "I'm not sure what" in text:
                        self.flag("Vader failed to recall earlier conversation — memory issue")

            # Response references Samantha?
            if "samantha" in text.lower():
                self.flag("Vader referenced 'Samantha' — should never mention her, he IS the assistant")
        ...
        if latest["role"] == "user":
            ...
            for pattern in complaint_patterns:
                if pattern in text:
                    self.flag(f"USER COMPLAINT detected: '{pattern}' — review Vader's previous response")
...
def main():
    ...
    print("🔍 Vader Conversation Monitor")
    ...
    try:
        for line in sys.stdin:
            ...
            # Parse Vader responses
            vader_match = re.search(r"Vader: (.+)$", line)
            if vader_match:
                text = vader_match.group(1)
                print(f"🤖 {text[:80]}{'...' if len(text) > 80 else ''}")
                monitor.add_message("vader", text)
